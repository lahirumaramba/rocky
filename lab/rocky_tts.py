...
DEMO_RESPONSE = os.getenv("DEMO_RESPONSE", "Amaze! This is a demo response from Rocky, question?")
DEMO_FILE = os.getenv("DEMO_FILE", "demo_phrases.txt")

# Piper TTS Config
USE_PIPER = os.getenv("USE_PIPER", "False").lower() in ["true", "1", "yes"]
PIPER_MODEL = os.getenv("PIPER_MODEL", "rocky_model_2999.onnx")
...
# PIPER_MODEL=en_US-lessac-low.onnx  | PIPER_RATE=16000
...
class RockyApp:
    def __init__(self):
        ...
        # Initialize Demo Phrases
        self.demo_phrases = []
        self.demo_index = 0
        if DEMO_MODE and DEMO_FILE:
            try:
                file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), DEMO_FILE)
                if os.path.exists(file_path):
                    with open(file_path, "r") as f:
                        # Load non-empty lines as phrases
                        self.demo_phrases = [line.strip() for line in f if line.strip()]
                    print(f"🧪 [Demo Mode] Loaded {len(self.demo_phrases)} phrases from {DEMO_FILE}")
                else:
                    # Fallback to current directory if not absolute and not found relative to script
                    if not os.path.isabs(DEMO_FILE) and os.path.exists(DEMO_FILE):
                         with open(DEMO_FILE, "r") as f:
                            self.demo_phrases = [line.strip() for line in f if line.strip()]
                         print(f"🧪 [Demo Mode] Loaded {len(self.demo_phrases)} phrases from {DEMO_FILE}")
            except Exception as e:
                print(f"Error loading demo phrases: {e}")
    ...
    def _stop_recording(self):
        ...
        # Process with LLM
        if DEMO_MODE:
            print("🧪 [Demo Mode] Bypassing network call...")
            if self.demo_phrases:
                response_text = self.demo_phrases[self.demo_index]
                print(f"🧪 [Demo Mode] Using phrase {self.demo_index + 1}/{len(self.demo_phrases)}")
                self.demo_index = (self.demo_index + 1) % len(self.demo_phrases)
            else:
        ...
        if response_text:
            ...
            # Start Piper speech (Non-blocking)
            if self.tts:
                self.tts.speak(response_text)
            
            # This blocks until Rocky's Eridian musical chords finish
